<template>
	<div>
        ...
        <div v-else class="item-body d-flex bg-white shadow-sm">
          <!-- <a class="d-block" target="_blank"><img src="/html/images/act/2021/01/01/images/coming-soon.jpg" class="img-fluid w-100" alt="商品即將開賣"></a> -->
          <a class="d-block" target="_blank"><img :src="item.imgsrc" @error="altImage" class="img-fluid w-100" :alt="item.id" :title="item.name"></a>
				...
				<div v-if="item.available" class="item-footer d-flex flex-column flex-grow-1 shadow-sm">
					<div class="item-brand p-2 bg-ef0f41"><a :href="item.url" class="text-white" target="_blank">{{item.brand}}</a></div>
					...
					<div class="item-price p-2 bg-white dollar-sign"><span>{{item.price}}</span></div>
				</div>
        <div v-else class="item-footer d-flex flex-column flex-grow-1 shadow-sm">
          <div class="item-brand p-2 bg-ef0f41"><span class="text-white" target="_blank">{{item.brand}}</span></div>
          <div class="item-name p-2 bg-light d-flex flex-grow-1 justify-content-center align-items-center"><span class="text-dark" target="_blank">{{item.name}}</span></div>
          <div class="item-price p-2 bg-white"><span class="coming-soon"><span class="text-secondary font-weight-normal"></span> {{item.launch}} <span class="text-secondary font-weight-normal">開賣</span></span></div>
...
module.exports = {
  data: function() {
    return {
      isLoading: false,
      failed: false,
      // imgError: 'this.src="/html/images/act/2021/01/01/images/img-error.jpg"',
      items: []
    };
  },
  methods: {
    initItem: function() {
      var self = this;
      var path1 = (window.location.hostname.includes("github.io") ? "/portfolio" : "") + "/html/images/act/2021/01/01/json/data.json";
      var path2 = (window.location.hostname.includes("github.io") ? "/portfolio" : "") + "/html/images/act/2021/01/01/json/data2.json";
      var path;

      let timeFromURL = this.getUrlParameter('t');

      if (timeFromURL) {
        timeFromURL = timeFromURL.substr(0, 8);  
      }
      ...
      // console.log('temp: ' + temp);
        ...
        if (timeFromURL == '20210216') {
          path = path2;
        } else {
          if (temp == '0216' || temp == '0217' || temp == '0218') {
            path = path2;
          } else {
            path = path1;
          }
    ...
    generateURL: function() {
      for (let i=0; i < this.items.length; i++) {
        ...
        // this.$set(this.items[i], "imgsrc", '/html/images/act/2021/01/01/images/product/'+ this.items[i].id +'/1.jpg');
      }
    },
    checkDate: function() {
      moment.locale('zh-tw');
    ...
    altImage: function(e) {
      ...
      // e.target.src = "/html/images/act/2021/01/01/images/img-error.jpg";

      if (e.target.imgchecked) {
        e.target.src = "/html/images/act/2021/01/01/images/img-error.jpg";
      } else {
        e.target.src = "/html/images/act/2021/01/01/images/product/"+ t +".jpg";
      }
